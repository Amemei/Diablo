...
function kisekaeRefresh(actor,armor){
    ...
    var costumeId = 31;
    var a, b, c;
    a = ($gameActors.actor(1).equips()[1]) ? $gameActors.actor(1).equips()[1].id : 0;    // 上半身装備
    b = ($gameActors.actor(1).equips()[2]) ? $gameActors.actor(1).equips()[2].id : 0;    // 下半身装備
    c = ($gameActors.actor(1).equips()[3]) ? $gameActors.actor(1).equips()[3].id : 0;    // 全身装備
    if (c != 0) {
        // 全身装備 = 防具ID
        ...
        $gameActors.actor(1).setCharacterImage("Illumina01", 0);
    } else if (a == 3 && b == 0) {
        // ブラ・白のみ
